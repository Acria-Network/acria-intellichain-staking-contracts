...
contract Staking {
    ...
    address[] private _stakedAddresses;
    mapping(address => uint256) public _addressToStakedAddressIndex;
    ...
    modifier onlyAdmin() {
        require(msg.sender == _admin, "Only admin can call function");
        _;
    }
    ...
    function setThreshold(uint256 value) public onlyAdmin {
        _validatorThreshold = value;

        address[] memory tempValidators;
        tempValidators = _stakedAddresses;

        for (uint i = 0; i < tempValidators.length; i++) {
            if(_addressToStakedAmount[tempValidators[i]] < _validatorThreshold) {
                _deleteFromValidators(tempValidators[i]);
            } else if (_canBecomeValidator(tempValidators[i])) {
                _appendToValidatorSet(tempValidators[i]);
            }
        }        
    }
    ...
    function _stake() private {
        ...
        if (_canBecomeValidator(msg.sender)) {
            ...
            _addressToStakedAddressIndex[msg.sender] = _stakedAddresses.length;
            _stakedAddresses.push(msg.sender);
        }
    ...
    function _unstake() private {
        ...
        if (_isValidator(msg.sender)) {
            ...
            uint256 index = _addressToStakedAddressIndex[msg.sender];
            uint256 lastIndex = _stakedAddresses.length - 1;

            if (index != lastIndex) {
                // exchange between the element and last to pop for delete
                address lastAddr = _stakedAddresses[lastIndex];
                _stakedAddresses[index] = lastAddr;
                _addressToStakedAddressIndex[lastAddr] = index;
            }

            _stakedAddresses.pop();
        }
